            ...
            {items.map((item) => (
              <div key={item.id} className="flex items-end gap-2">
                <div className="flex-1 space-y-1.5">
                  {item.id === items[0].id && <Label>Description</Label>}
                  <Input name="description" defaultValue={item.description} placeholder="Custom island cabinetry" />
                </div>
                <div className="w-20 space-y-1.5">
                  {item.id === items[0].id && <Label>Qty</Label>}
                  <Input name="quantity" type="number" step="0.1" defaultValue={item.quantity} />
                </div>
                <div className="w-28 space-y-1.5">
                  {item.id === items[0].id && <Label>Unit price</Label>}
